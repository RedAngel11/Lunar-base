...
BaseCalculator::MaterialProps BaseCalculator::getCustomMaterialProps(const CustomMaterialParams& p) {
    return {"Пользовательский материал", p.density, p.thermalConductivity, p.radiationShielding,
...
CalculationReport BaseCalculator::calculate(const SelectionData& data) {
    ...
    // Берем первый (и теперь единственный) материал
    ...
    switch (sp.type) {
        case StructureType::UndergroundBunker:
            effectiveBurial += 0.4;
            structRadiationMod = 0.6; structThermalMod = 0.7; structDurabilityMod = 1.2;
            break;
        case StructureType::InflatableDome:
            structRadiationMod = 1.4; structThermalMod = 1.2; structDurabilityMod = 0.7;
            effectiveBurial *= 0.5;
            break;
        case StructureType::RegolithPrinted:
            structRadiationMod = 0.8; structThermalMod = 0.9; structDurabilityMod = 1.3;
            effectiveBurial += 0.2;
            break;
        default: break;
    }
    ...
    if (sp.type == StructureType::UndergroundBunker) costRes.totalCost *= 1.15;
    if (sp.type == StructureType::InflatableDome) costRes.totalCost *= 0.85;
    ...
    if (lifespan < 10.0) recs.push_back("Срок службы низкий. Увеличьте толщину стенок или добавьте реголит.");
    if (radiationRisk > 5.0) recs.push_back("Высокий радиационный риск. Рекомендуется заглубление или аэрогель.");
    if (thermalStress > 200.0) recs.push_back("Высокий термический стресс. Используйте изоляционные материалы.");
...
std::ostream& operator<<(std::ostream& os, const CalculationReport& r) {
    os << "ИТОГОВЫЙ ОТЧЁТ\n"
       ...
       << "Стенка: " << r.wallThickness << " м\n"
       << "Объём: " << r.totalVolume << " м3\n\n"
       << "Стоимость: $" << (r.totalCost/1e6) << " млн\n"
       << "Масса с Земли: " << r.earthLaunchMass << " кг\n"
       << "Масса на Луне: " << r.localMass << " кг\n\n"
       << "Срок службы: " << r.estimatedLifespan << " лет\n"
       << "Безопасность: " << static_cast<int>(r.safetyScore) << "/100\n\n"
       << "Рекомендации:\n";
